...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        self.y = tf.placeholder('int32', [N, None, None], name='y')
        self.qID = tf.placeholder('string', [N, None, None], name='qID')

        self.correctIndex = tf.placeholder('int32', [N, None, None], name='correctIndex')
        self.Pos1 = tf.placeholder('float', [N, None, None], name='Pos1')
        self.Neg1 = tf.placeholder('float', [N, None, None], name='Neg1')
        self.mask1 = tf.placeholder('bool', [N, None, None], name='mask1')
        self.invmask1 = tf.placeholder('bool', [N, None, None], name='invmask1')

        self.y2 = tf.placeholder('int32', [N, None, None], name='y2')
        self.wy = tf.placeholder('int32', [N, None, None], name='wy')
    ...
    def _build_loss(self):
        ...
        if config.wy:
            ...
            if config.na:
                ...
                #    logits=self.logits, labels=tf.cast(tf.reshape(self.y, [-1, M * JX]), 'float'))
                #losses2 = tf.losses.hinge_loss(
                #    logits=self.logits2, labels=tf.cast(tf.reshape(self.y2, [-1, M * JX]), 'float'))


                intermediateLabels1 = tf.cast(tf.reshape(self.y, [-1, M * JX]), 'float')
                correctIndex = tf.argmax(tf.cast(tf.equal(intermediateLabels1, 1), tf.int32), axis=1)

                correctIndex = tf.cast(correctIndex, tf.int32)
                correctIndex = tf.reshape(correctIndex,(N,1))
                self.correctIndex = correctIndex
                rangMat = tf.range(0,N)
                rangMat = tf.reshape(rangMat, (N, 1))
                indexingMat = tf.concat([rangMat,correctIndex],1)

                maskingMat = tf.sparse_to_dense(indexingMat,tf.shape(intermediateLabels1),1)
                maskingMat = tf.cast(maskingMat, 'bool')

                #inverMaskingMat = -1*(maskingMat - 1)
                inverMaskingMat = tf.logical_not(maskingMat)
                self.mask1 = maskingMat
                self.invmask1 = inverMaskingMat

                scoresPos1 = tf.boolean_mask(self.logits, maskingMat)
                scoresNeg1 = tf.boolean_mask(self.logits, inverMaskingMat)

                scoresPos1 = tf.reshape(scoresPos1, (N, 1))
                scoresNeg1 = tf.reshape(scoresNeg1, (N, -1))#M * JX - 1))
                self.Pos1 = scoresPos1
                self.Neg1 = scoresNeg1
                #result = (1 - scoresPos1) + scoresNeg1
                loss_matrix = tf.maximum(0., (1. - scoresPos1) + scoresNeg1)
                losses = tf.reduce_sum(loss_matrix,axis=1)


                intermediateLabels2 = tf.cast(tf.reshape(self.y2, [-1, M * JX]), 'float')
                correctIndex = tf.argmax(tf.cast(tf.equal(intermediateLabels2, 1), tf.int32), axis=1)
                #
                correctIndex = tf.cast(correctIndex, tf.int32)
                correctIndex = tf.reshape(correctIndex,(N,1))
                #self.correctIndex = correctIndex
                ...
                indexingMat = tf.concat([rangMat,correctIndex],1)
                #
                maskingMat = tf.sparse_to_dense(indexingMat,tf.shape(intermediateLabels2),1)
                maskingMat = tf.cast(maskingMat, 'bool')
                inverMaskingMat = tf.logical_not(maskingMat)
                #
                # self.mask1 = maskingMat
                # self.invmask1 = inverMaskingMat
                #
                scoresPos2 = tf.boolean_mask(self.logits2, maskingMat)
                scoresNeg2 = tf.boolean_mask(self.logits2, inverMaskingMat)
                #
                scoresPos2 = tf.reshape(scoresPos2, (N, 1))
                scoresNeg2 = tf.reshape(scoresNeg2, (N, -1))#M * JX - 1))
                # self.Pos1 = scoresPos2
                # self.Neg1 = scoresNeg2
                # #result = (1 - scoresPos1) + scoresNeg1
                loss_matrix2 = tf.maximum(0., (1. - scoresPos2) + scoresNeg2)
                losses2 = tf.reduce_sum(loss_matrix2,axis=1)
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        if supervised:
            ...
            y2 = np.zeros([N, M, JX], dtype='int32')
            wy = np.zeros([N, M, JX], dtype='int32')
            na = np.zeros([N], dtype='int32')
            feed_dict[self.y] = y
            ...
            for i, (xi, cxi, yi, nai) in enumerate(zip(X, CX, batch.data['y'], batch.data['na'])):
                ...
                if j == j2:
                    ...
                    wy[i, j, k:len(batch.data['x'][i][j])] = 1
                    wy[i, j2, :k2] = 1
...
def getPredictionScore(questionId, startIndex, endIndex):
    return