...
class MultiGPUTrainer(object):
    ...
    def step(self, sess, batches, get_summary=False):
        ...
        if get_summary:
            loss, summary, train_op, logits, logits2, tensorDic, y, y2,wy,yp,yp2,wyp,correctIndex, Pos1,Neg1,\
                mask1, invmask1\
                = sess.run([self.loss, self.summary, self.train_op,
                          self.logits, self.logits2, self.tensorDic,
                          self.y, self.y2,self.wy, self.yp,self.yp2,self.wyp,self.correctIndex,self.Pos1,self.Neg1,
                            self.mask1, self.invmask1], feed_dict=feed_dict)
        else:
            loss, train_op, logits, logits2,tensorDic, y, y2,wy,yp,yp2,wyp, correctIndex,Pos1,Neg1, \
                mask1, invmask1\
                = sess.run([self.loss, self.train_op, self.logits,
                          self.logits2, self.tensorDic,
                          self.y, self.y2, self.wy, self.yp, self.yp2, self.wyp, self.correctIndex,self.Pos1,self.Neg1,
                            self.mask1, self.invmask1], feed_dict=feed_dict)
            summary = None
        return loss, summary, train_op
